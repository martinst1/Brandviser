...
namespace Brandviser.Tests.Controllers.Seller_Area.SellerControllerTests
{
    ...
    public class Rejected_Should
    {
        ...
        public void Call_LoggedInUser_GetUserId_Once()
        {
            ...
            domainService.Setup(d => d.GetSellerRejectedDomainsByUserId(userId)).Returns(domains.AsQueryable<Domain>());

            // Act
            sellerController.Rejected();
        ...
        public void RenderDefaultView_WithExpectedViewModel()
        {
            ...
            // Act & Assert
            sellerController
                .WithCallTo(s => s.Rejected())
                .ShouldRenderPartialView("_Rejected")
                .WithModel<IEnumerable<PartialDomainViewModel>>(
                p =>
                {
                    Assert.AreEqual(1, p.Count());
                    Assert.AreEqual("name", p.First().Name);
                    Assert.AreEqual("Rejected", p.First().Status);
                });
        }
    }
}
